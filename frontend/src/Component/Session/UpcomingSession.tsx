...
const UpcomingSession = ({ schedule, editMode, list, type }: Props) => {
    ...
    const [exerciseToEdit, setExerciseToEdit] = useState<Exercise>(currentExercises[0])
    ...
    return (
        <>
                ...
                <div className='d-flex flex-column Routine-wrapper-top'>
                    <div className='d-flex justify-content-between'>
                        <div className='flex-1 d-flex gap-1'>
                            <span className='Routine-title Routine-title-number text-black semi-bold desktop-medium'>{list}.</span>
                            <span className='Routine-title Routine-title-name text-black semi-bold desktop-medium'>{schedule.routine.name}</span>
                        </div>
                        {
                            editMode &&
                            (
                        ...
                        schedule.routine.exercises.map((exercise, index) => {
                            return (
                                <div key={index} className='d-flex align-items-start gap-2' >
                                    <button
                                        className={`Exercise ${exercise.complete ? 'completed' : ''} border-0 bg-transparent pe-0 position-relative color-typography-secondary desktop-small regular`}
                                        onClick={() => {
            ...
            <EditWorkoutModal scheduleItem={schedule} show={modalShow} onHide={() => setModalShow(false)} exercise={exerciseToEdit} />
        </>
