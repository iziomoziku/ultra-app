...
const EditWorkout = ({ show, onHide, exercise, scheduleItem }: Props) => {
    const { schedule, addExerciseSet, deleteExerciseSet, markExerciseAsComplete } = useMainContext();
    ...
    useEffect(() => {
        ...
        const foundExercise = foundScheduleItem?.routine.exercises.find(e => e.id === exercise.id);
    ...
    const handleSetChange = (id: string, field: "rep" | "weight", value: string) => {
    ...
    return (
        <Modal
            ...
            centered
        >
            <Modal.Header closeButton className="p-0 border-0 d-flex align-items-start">
                ...
                <div className="d-flex flex-column gap-18">
                    <div>
                        <div className="d-flex justify-content-between edit-workout-header-row">
                            <span className="desktop-medium medium color-typography-additional">#</span>
                            <div className="d-flex justify-content-end align-items-center quantity-weight-header">
                                ...
                                <button
                                    ...
                                    onClick={() => addExerciseSet(scheduleItem.id, updatedExercise.id)}
                                >
                        ...
                        <div className="data-cell">
                            {exerciseSets.map(({ id, rep, weight }, index) => {
                                // const [rep, weight] = r.rep.split('x');
                                        ...
                                        <div className="d-flex justify-content-end align-items-center quantity-weight-data-cell">
                                            <div className="d-flex justify-content-end quantity-weight-data-cell-wrapper">
                                                <input
                                                    ...
                                                    type="number"
                                                    value={rep}
                                                    onChange={(e) => handleSetChange(id, "rep", e.target.value)}
                                                />
                                                <input
                                                    className="desktop-xtra-small medium color-typography-additional edit-workout-quantity border-0 bg-grey-2 text-center"
                                                    type="number"
                                                    value={weight}
                                                    onChange={(e) => handleSetChange(id, "weight", e.target.value)}
                                                />
                                            </div>
                                            <button className='bg-transparent border-0' onClick={() => deleteExerciseSet(updatedExercise.id, id)}>
            ...
            </div>
            {
                ...
                (
                    <Modal.Footer className="border-0 d-flex m-0 p-0">
                        <Button className="flex-1 button-varriant-2 desktop-medium medium" onClick={updateExercise}>
                            Complete
                        </Button>
                    </Modal.Footer>
                )
